...
class _GrainDetailScreenState extends State<GrainDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    bool isFav = favoritesProvider.isFavorite(widget.grain['name']);
    return Scaffold(
      appBar: AppBar(),
      body: SafeArea(
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        var favoriteProvider = Provider.of<FavoritesProvider>(
                            ...
                            listen: false);
                        if (isFav) {
                          favoriteProvider.removeFavorite(widget.grain['name']);
                        } else {
                          favoriteProvider.addFavorite({
                            'name': widget.grain['name'],
                            'image': widget.grain['image'],
                            'price': widget.grain['price'],
                          });
                        }
                      },
                    )
              ...
              Padding(
                ...
                child: GestureDetector(
                  onTap: () {
                    ...
                    // Convert price
                    double price = double.tryParse(widget.grain['price']
                                .replaceAll(RegExp(r'[^0-9]'), ''))
                            ?.toDouble() ??
                        0.0;

                    cartProvider.addToCart(
                      widget.grain['id'].toString(),
                      widget.grain['name'].toString(),
                      price,
                      quantity,
                      widget.grain['image'].toString(),
                    );

                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                          content:
                              Text("${widget.grain['name']} added to cart!")),
                    );
                  },
                  child: Container(
                    width: double.infinity,
